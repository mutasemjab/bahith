...
@section('title', 'تعديل جدول حصص')

@section('content')

<div class="page-header d-flex align-items-start justify-content-between flex-wrap gap-3">
    <div><h1 class="page-title">تعديل جدول حصص</h1></div>
...
<form action="{{ route('admin.class-schedules.update', $classSchedule->id) }}" method="POST" enctype="multipart/form-data">
@csrf @method('PUT')
    ...
    <div class="panel-card-body">
        <div class="row g-3">
            ...
            <div class="col-12">
                ...
                <select name="class_id" class="form-control @error('class_id') is-invalid @enderror" required>
                    @foreach($classes as $class)
                        <option value="{{ $class->id }}" {{ old('class_id', $classSchedule->class_id) == $class->id ? 'selected' : '' }}>
                            {{ $class->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="col-12">
                ...
                <img src="{{ asset('assets/uploads/class-schedules/' . $classSchedule->image) }}"
                     id="previewImg"
